...
static const char** EndDigits = nullptr;

void GenRevDigits()
{
	EndDigits = new const char*[ARRAY_LEN(Digits)];
	
	for (int i=0; i<ARRAY_LEN(Digits); i++)
	{
		EndDigits[i] = Digits[i];
...
int FirstAlphaNumDigit(const char* Start)
{
	while (*Start < '0' || *Start > '9')
	{
		for (int i=0; i<ARRAY_LEN(Digits); i++)
		{
			if (EqualsFront(Start, Digits[i]))
				return i+1;
		}
...
int LastAlphaNumDigit(const char* End)
{
	while (*End < '0' || *End > '9')
	{
		for (int i=0; i<ARRAY_LEN(Digits); i++)
		{
			if (EqualsBack(End, EndDigits[i], Digits[i]))
				return i+1;
		}
...
void Day01()
{
	...
	const char *End = Puzzle;
	
	int Sum = 0;
	while (ReadNextLine(&Start, &End))
	{
		//printf("%i %i\n", FirstDigit(Start), LastDigit(End));
		Sum += FirstDigit(Start) * 10 + LastDigit(End);
	}
	...
	while (ReadNextLine(&Start, &End))
	{
		//printf("%i %i\n", FirstAlphaNumDigit(Start), LastAlphaNumDigit(End));
		Sum += FirstAlphaNumDigit(Start) * 10 + LastAlphaNumDigit(End);
	}
