...
.........................610.26.............892...............................299............601............721..729........................)MEOW";

static bool IsD4Symbol(char c)
{
	return c != '.' && (c < '0' || c > '9');
}

static void AddD4Marks(ACString Sz, char *markBuffer)
{
	if (Sz.Start == nullptr) return;
	
	int i = 0;
	while (Sz.Start != Sz.End)
	{
		if (IsD4Symbol(*Sz.Start))
		{
			markBuffer[i] = '*';
			if (i != 0)
				markBuffer[i-1] = '*';
...
void CountGears(ACString Src, int &count, int &product, int index)
{
	if (Src.Start == nullptr)	return;
	...
	while (Src.Start < Src.End)
	{
		...
		while (IsNumeric(*Src.Start) && Src.Start < Src.End)
		{
			foundNumber = true;
			number *= 10;
			number += (*Src.Start - '0');
...
void Day03()
{
	...
	Sz[0] = Sz[1] = Sz[2] = MakeACString(Puzzle);
	...
	while (*Sz[1].End != 0)
	{
		...
		if (Sz[1].Start == nullptr)	continue;
		
		//PrintLine(Sz[1]);
		
		// Very much mechanical...
		char markCache[256] = {0};
		for (int i=0; i<3; i++)
			AddD4Marks(Sz[i], markCache);
		
		// Now find all the numbers...
		ACString Cp = Sz[1];
		int i=0;
		while (Cp.Start < Cp.End)
		{
			int number = 0;
			bool foundMark = false;
			while (IsNumeric(*Cp.Start) && Cp.Start < Cp.End)
			{
				number *= 10;
				number += (*Cp.Start - '0');
				if (markCache[i] != 0)
		...
		while (Cp2.Start != Cp2.End)
		{
			if (*Cp2.Start == '*')
			{
				...
				int product = 1;
				for (int k=0; k<3; k++)
					CountGears(Sz[k], count, product, index);
