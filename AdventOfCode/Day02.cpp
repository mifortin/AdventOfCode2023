...
R"MEOW(Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
Game 2: 1 blue, 2 green; 3 green, 4 blue, 1 red; 1 green, 1 blue
Game 3: 8 green, 6 blue, 20 red; 5 blue, 4 red, 13 green; 5 green, 1 red
Game 4: 1 green, 3 red, 6 blue; 3 green, 6 red; 3 green, 15 blue, 14 red
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green)MEOW";

static const char* Puzzle =
R"MEOW(Game 1: 8 green, 4 red, 4 blue; 1 green, 6 red, 4 blue; 7 red, 4 green, 1 blue; 2 blue, 8 red, 8 green
...
Game 100: 5 green, 17 blue, 5 red; 15 blue; 13 green, 8 red, 3 blue; 16 blue, 15 green, 8 red; 16 green, 2 blue, 3 red)MEOW";
...
{
	return ReadNextLine(&Sz.Start, &Sz.End);
}

void PrintLine(ACString &sz)
{
	PrintLine(sz.Start, sz.End);
}


void SkipWhitespace(ACString &sz)
{
	while (sz.Start != sz.End && (*sz.Start == ' ' || *sz.Start == '\t'))
		sz.Start++;
}
...
{
	SkipWhitespace(sz);
	if (EqualsFront(sz.Start, String))
	{
		sz.Start += strlen(String);
		return true;
	}
...
void Day02()
{
	...
	while(ReadNextLine(Sz))
	{
		...
		int Counts[ARRAY_LEN(Colours)] = {0};
		int Minimums[ARRAY_LEN(Colours)] = {0};
		int GameNum;
		Consume(Sz, "Game");
		Consume(Sz, GameNum);
		Consume(Sz, ":");
		...
		bool bPossibleOne = true;
		while (Consume(Sz, Count))
		{
			for (int i=0; i<ARRAY_LEN(Colours); i++)
			{
				if (Consume(Sz, Colours[i]))
				{
					Counts[i] += Count;
					...
					Minimums[i] = std::max(Minimums[i], Count);
				}
			}
		...
		bool bPossibleSum = true;
		for (int i=0; i<ARRAY_LEN(Colours); i++)
		{
			product *= Minimums[i];
		//	printf(" > %s = %i <= %i\n", Colours[i], Counts[i], MaxCounts[i]);
			if (Counts[i] > MaxCounts[i])
			{
				//printf(" %i > %s = %i <= %i\n", GameNum, Colours[i], Counts[i], MaxCounts[i]);
				bPossibleSum = false;
			}
		}
		//printf("%i %i\n", GameNum, product);
